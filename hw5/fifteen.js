...
$(document).ready(function() {
 ...
 let topPos = getCurTopPos("#puzzlearea")-150; //Incorrect if not minus 
...
function mouseOut(){
 ...
 //if the piece is a neigbor of the empty piece, 
 //the highlight attributes will be removed
 if(movablePiece(row, col) == 1){
...
function movablePiece(row, col){
 if((((row+1) == emptyPiece_row) || (((row-1) == emptyPiece_row))) && 
		(col == emptyPiece_col)){
  return 1;
 }
 else{
  if((((col+1) == emptyPiece_col) || (((col-1) == emptyPiece_col))) &&
		(row == emptyPiece_row)){
   return 1;
  }
 }
...
function movePiece(piece, row, col){
 //checking the moved piece must have the same row OR the same column of the empty piece
 if((row == emptyPiece_row || col == emptyPiece_col)){
  //able to move down to the emply piece at the next row
  if(((row+1)<rows_cols) && ((row+1) == emptyPiece_row)){ 
